---
id: PERSON-LEE-TIEN
title: Lee Tien
type: attorney
status: draft
summary: Lee Tien is Legislative Director and Adams Chair for Internet Rights at the Electronic Frontier Foundation, leading EFF's legislative advocacy on privacy, surveillance, and internet policy.
tags:
  - attorney
  - privacy
  - digital-rights
  - internet-governance
sources:
  - SRC-TIEN-EFF
relationships:
  - subject: PERSON-LEE-TIEN
    predicate: cites
    object: SRC-TIEN-EFF
    sources:
      - SRC-TIEN-EFF
  - subject: PERSON-LEE-TIEN
    predicate: affiliated_with
    object: ORG-EFF
    sources:
      - SRC-TIEN-EFF
last_verified: "2026-07-04"
---

# Generated Source Mirror

This page is generated from the source Markdown file so it can be viewed inside the MkDocs site.

- Source path: `knowledge/people/PERSON-LEE-TIEN.md`
- Source ID: `PERSON-LEE-TIEN`
- [**Edit this page**](https://github.com/itsmeront/open-internet-reference/edit/main/knowledge/people/PERSON-LEE-TIEN.md) | [**Suggest a change**](https://github.com/itsmeront/open-internet-reference/issues/new?template=suggest-edit.yml&title=Edit+suggestion:+PERSON-LEE-TIEN)

!!! info "📝 Page Status: Draft — sourced but not yet reviewed"

    - **Status**: `draft`
    - **Sources**: 1
    - **Relationships**: 2
    - **Research debt items**: 4
    - **Last verified**: 2026-07-04

---

# Lee Tien

## Summary

Lee Tien is Legislative Director and the Adams Chair for Internet Rights at the Electronic Frontier Foundation. He leads EFF's legislative advocacy on privacy, surveillance, and internet policy rather than serving as a senior staff litigator.

## Verified Facts

- The official EFF staff profile describes Tien as Legislative Director and Adams Chair for Internet Rights.[^1]
- Before joining EFF, he was a sole practitioner specializing in Freedom of Information Act litigation.[^1]
- He received his undergraduate degree from Stanford University and his law degree from UC Berkeley Law School.[^1]
- He did graduate work in the Program in Jurisprudence and Social Policy at UC Berkeley.[^1]
- Before law school, he worked as a news reporter at the Tacoma News Tribune.[^1]
- The official profile lists email lee@eff.org and phone +1 415 436 9333 x102.[^1]
- His published EFF work includes advocacy on federal privacy legislation, HIPAA modifications, California Consumer Privacy Act implementation, and cross-border data access proposals.[^1]


[^1]: [`SRC-TIEN-EFF`](../../../bibliography.md#SRC-TIEN-EFF) — Lee Tien EFF Staff Biography. Self-reported profile; corroboration pending.

## Relevance to Open Source and Software Companies

Tien's legislative work on privacy bills, cross-border data access, and consumer privacy regulation shapes the statutory environment in which software companies operate. Companies facing surveillance reform, HIPAA, or state privacy compliance questions may engage EFF's legislative team through policy advocacy rather than direct litigation intake.

## Relationships

- `PERSON-LEE-TIEN` cites [`SRC-TIEN-EFF`](../../../bibliography.md#SRC-TIEN-EFF).
- `PERSON-LEE-TIEN` affiliated_with `ORG-EFF`.

## Sources

- [`SRC-TIEN-EFF`](../../../bibliography.md#SRC-TIEN-EFF): Lee Tien EFF Staff Biography.

## Research Debt

- Distinguish legislative engagement paths from EFF litigation intake.
- Document specific bills and congressional testimony.
- Corroborate self-reported biography or about-page claims with independent sources (institutional profile, court docket, case page, or reputable journalism).
- Add primary or secondary sources for major litigation and impact claims (dockets, case pages, official court records, or reputable journalism).
