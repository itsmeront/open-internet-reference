---
id: PERSON-KATIE-FALLOW
title: Katie Fallow
type: attorney
status: draft
summary: Katie Fallow is deputy litigation director at the Knight First Amendment Institute, where she litigates free speech and press issues in digital spaces including government use of social media.
tags:
  - attorney
  - first-amendment
  - digital-rights
sources:
  - SRC-FALLOW-KNIGHT
  - SRC-BROWN-V-EMA-LII
relationships:
  - subject: PERSON-KATIE-FALLOW
    predicate: cites
    object: SRC-FALLOW-KNIGHT
    sources:
      - SRC-FALLOW-KNIGHT
  - subject: PERSON-KATIE-FALLOW
    predicate: cites
    object: SRC-BROWN-V-EMA-LII
    sources:
      - SRC-BROWN-V-EMA-LII
  - subject: PERSON-KATIE-FALLOW
    predicate: affiliated_with
    object: ORG-KNIGHT-FIRST-AMENDMENT
    sources:
      - SRC-FALLOW-KNIGHT
  - subject: PERSON-KATIE-FALLOW
    predicate: related_to
    object: CASE-BROWN-V-ENTERTAINMENT-MERCHANTS
    sources:
      - SRC-FALLOW-KNIGHT
      - SRC-BROWN-V-EMA-LII
last_verified: "2026-07-04"
---

# Generated Source Mirror

This page is generated from the source Markdown file so it can be viewed inside the MkDocs site.

- Source path: `knowledge/people/PERSON-KATIE-FALLOW.md`
- Source ID: `PERSON-KATIE-FALLOW`
- [**Edit this page**](https://github.com/itsmeront/open-internet-reference/edit/main/knowledge/people/PERSON-KATIE-FALLOW.md) | [**Suggest a change**](https://github.com/itsmeront/open-internet-reference/issues/new?template=suggest-edit.yml&title=Edit+suggestion:+PERSON-KATIE-FALLOW)

!!! info "📝 Page Status: Draft — sourced but not yet reviewed"

    - **Status**: `draft`
    - **Sources**: 2
    - **Relationships**: 4
    - **Research debt items**: 6
    - **Last verified**: 2026-07-04

---

# Katie Fallow

## Summary

Katie Fallow is deputy litigation director at the Knight First Amendment Institute, where she focuses on threats to free speech and a free press in the digital age, particularly government use of social media.

## Verified Facts

- The official Knight Institute biography describes Fallow as deputy litigation director.[^1]
- She spearheads the Institute's litigation concerning the government's use of social media.[^1]
- She was one of the lead lawyers in the Institute's challenge to President Trump's blocking of people from his @realDonaldTrump Twitter account.[^1]
- She won the first federal appellate case holding that public officials who use social media accounts for official purposes have created a public forum and may not block people based on viewpoint.[^1]
- She is litigating the Institute's challenges to regulations requiring individuals to provide social media handles to the government for collection and surveillance.[^1]
- Before joining the Knight Institute, Fallow was a litigation partner at Jenner & Block in Washington, D.C., where she represented video game makers in First Amendment challenges culminating in [Brown v. Entertainment Merchants Assn.](../legal/CASE-BROWN-V-ENTERTAINMENT-MERCHANTS.md) (*EMA v. Brown*) at the Supreme Court.[^1][^2]
- She later served as deputy director of the Bureau of Consumer Protection at the Federal Trade Commission.[^1]
- She graduated from the University of California at Berkeley and Harvard Law School.[^1]


[^1]: [`SRC-FALLOW-KNIGHT`](../../../bibliography.md#SRC-FALLOW-KNIGHT) — Katie Fallow Knight Institute Biography. Self-reported profile; corroboration pending.
[^2]: [`SRC-BROWN-V-EMA-LII`](../../../bibliography.md#SRC-BROWN-V-EMA-LII) — Brown v. Entertainment Merchants Assn., 564 U.S. 786 (2011) (Cornell LII). Primary authority.

## Relevance to Open Source and Software Companies

Fallow's litigation on digital public forums, government social media blocking, and surveillance of online identities is relevant when software platforms, developers, or companies face government speech restrictions or data collection mandates tied to online accounts.

## Relationships

- `PERSON-KATIE-FALLOW` cites [`SRC-FALLOW-KNIGHT`](../../../bibliography.md#SRC-FALLOW-KNIGHT).
- `PERSON-KATIE-FALLOW` cites [`SRC-BROWN-V-EMA-LII`](../../../bibliography.md#SRC-BROWN-V-EMA-LII).
- `PERSON-KATIE-FALLOW` affiliated_with `ORG-KNIGHT-FIRST-AMENDMENT`.
- `PERSON-KATIE-FALLOW` related_to `CASE-BROWN-V-ENTERTAINMENT-MERCHANTS`.

## Sources

1. [`SRC-FALLOW-KNIGHT`](../../../bibliography.md#SRC-FALLOW-KNIGHT): Katie Fallow Knight Institute Biography.
2. [`SRC-BROWN-V-EMA-LII`](../../../bibliography.md#SRC-BROWN-V-EMA-LII): Brown v. Entertainment Merchants Assn., 564 U.S. 786 (2011) (Cornell LII).

## Research Debt

- Create `CASE-*` page and primary source for mentioned case `Knight Institute v. Trump`.
- Create `CASE-*` page and primary source for mentioned case `Doc Society v. Blinken` (social media registration challenges).
- Corroborate Fallow's Jenner & Block role in *Brown* with docket entries or secondary reporting beyond the Knight biography.
- Document post-FTC litigation focus areas.
- Corroborate self-reported biography or about-page claims with independent sources (institutional profile, court docket, case page, or reputable journalism).
- Add primary or secondary sources for major litigation and impact claims (dockets, case pages, official court records, or reputable journalism).
