---
id: PERSON-DANAH-BOYD
title: danah boyd
type: person
status: draft
summary: danah boyd is a partner researcher at Microsoft Research, founder of the Data & Society Research Institute, and Distinguished Visiting Professor at Georgetown University who studies social media, algorithmic bias, privacy, and the societal impact of data-driven systems.
tags:
  - person
  - privacy
  - internet-governance
  - digital-rights
sources:
  - SRC-BOYD-MSR
relationships:
  - subject: PERSON-DANAH-BOYD
    predicate: cites
    object: SRC-BOYD-MSR
    sources:
      - SRC-BOYD-MSR
last_verified: "2026-07-04"
---

# Generated Source Mirror

This page is generated from the source Markdown file so it can be viewed inside the MkDocs site.

- Source path: `knowledge/people/PERSON-DANAH-BOYD.md`
- Source ID: `PERSON-DANAH-BOYD`
- [**Edit this page**](https://github.com/itsmeront/open-internet-reference/edit/main/knowledge/people/PERSON-DANAH-BOYD.md) | [**Suggest a change**](https://github.com/itsmeront/open-internet-reference/issues/new?template=suggest-edit.yml&title=Edit+suggestion:+PERSON-DANAH-BOYD)

!!! info "📝 Page Status: Draft — sourced but not yet reviewed"

    - **Status**: `draft`
    - **Sources**: 1
    - **Relationships**: 1
    - **Research debt items**: 3

---

# danah boyd

## Summary

danah boyd is a partner researcher at Microsoft Research, founder of the Data & Society Research Institute, and Distinguished Visiting Professor at Georgetown University who studies social media, algorithmic bias, privacy, and the societal impact of data-driven systems.

## Verified Facts

- Partner Researcher at Microsoft Research (New England lab).[^1]
- Founder of Data & Society Research Institute, an independent research organization studying the social implications of technology.[^1]
- Distinguished Visiting Professor at Georgetown University (per Microsoft Research institutional announcements, 2023).[^1]
- PhD from the UC Berkeley School of Information; former MIT Media Lab research assistant (MS 2002).[^1]
- Former fellow at Harvard's Berkman Klein Center for Internet & Society.[^1]
- Author of *It's Complicated: The Social Lives of Networked Teens* and co-author of *Hanging Out, Messing Around, and Geeking Out*.[^1]
- Research focuses on youth and social media, media manipulation, algorithmic bias, privacy practices, and ethnographic study of data legitimacy (including U.S. census data systems).[^1]
- Recipient of MIT's Morison Prize in Science, Technology, and Society (2023).[^1]


[^1]: [`SRC-BOYD-MSR`](../../../bibliography.md#SRC-BOYD-MSR) — danah boyd Microsoft Research Profile. Official source; review reliability tier.

## Expert Testimony and Public Advocacy

boyd is frequently cited in policy debates on platform design, teen safety online, content moderation tradeoffs, and algorithmic accountability. Her ethnographic methodology helps courts and regulators understand how platform affordances shape user behavior — not merely what policy documents claim.

## Relevance to Lawsuits Involving Software and Internet Infrastructure

boyd is particularly useful when a case involves:

- Social media platform design and its effects on minors
- Algorithmic recommendation, bias, or manipulation claims
- Privacy expectations in networked publics
- Gap between stated platform policies and observed user practices

## Relationships

- `PERSON-DANAH-BOYD` cites [`SRC-BOYD-MSR`](../../../bibliography.md#SRC-BOYD-MSR).

## Sources

1. [`SRC-BOYD-MSR`](../../../bibliography.md#SRC-BOYD-MSR): danah boyd Microsoft Research Profile.

## Research Debt

- Add Data & Society institute profile as secondary source.
- Document specific congressional or regulatory testimony with primary citations.
- Link to relevant OIR topic pages on intermediary liability and platform governance.

## Document metadata

- Last verified: `2026-07-04`
