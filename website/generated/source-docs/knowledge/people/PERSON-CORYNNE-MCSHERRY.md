---
id: PERSON-CORYNNE-MCSHERRY
title: Corynne McSherry
type: attorney
status: draft
summary: Corynne McSherry is the Legal Director at the Electronic Frontier Foundation, specializing in intellectual property, copyright, intermediary liability, open access, and free speech issues.
tags:
  - attorney
  - copyright
  - digital-rights
  - intermediary-liability
  - speech-and-code
sources:
  - SRC-MCSHERRY-EFF-BIO
relationships:
  - subject: PERSON-CORYNNE-MCSHERRY
    predicate: cites
    object: SRC-MCSHERRY-EFF-BIO
    sources:
      - SRC-MCSHERRY-EFF-BIO
  - subject: PERSON-CORYNNE-MCSHERRY
    predicate: affiliated_with
    object: ORG-EFF
    sources:
      - SRC-MCSHERRY-EFF-BIO
last_verified: "2026-06-25"
---

# Generated Source Mirror

This page is generated from the source Markdown file so it can be viewed inside the MkDocs site.

- Source path: `knowledge/people/PERSON-CORYNNE-MCSHERRY.md`
- Source ID: `PERSON-CORYNNE-MCSHERRY`
- [**Edit this page**](https://github.com/itsmeront/open-internet-reference/edit/main/knowledge/people/PERSON-CORYNNE-MCSHERRY.md) | [**Suggest a change**](https://github.com/itsmeront/open-internet-reference/issues/new?template=suggest-edit.yml&title=Edit+suggestion:+PERSON-CORYNNE-MCSHERRY)

!!! info "📝 Page Status: Draft — sourced but not yet reviewed"

    - **Status**: `draft`
    - **Sources**: 1
    - **Relationships**: 2
    - **Research debt items**: 5
    - **Last verified**: 2026-06-25

---

# Corynne McSherry

## Summary

Corynne McSherry is the Legal Director at the Electronic Frontier Foundation, specializing in intellectual property, copyright, intermediary liability, open access, and free speech issues.

## Verified Facts

- The official EFF biography describes McSherry as the Legal Director at EFF.[^1]
- She specializes in intellectual property, copyright, intermediary liability, open access, and free speech issues.[^1]
- Her cases involve defending online fair use, political expression, and the public domain.[^1]
- As a litigator, she has represented the Internet Archive and Professor Lawrence Lessig, among others.[^1]
- She worked on the Sony-BMG rootkit litigation at EFF.[^1]
- She defended the Yes Men and other activists in a lawsuit filed by the U.S. Chamber of Commerce over a political parody.[^1]
- She has been at EFF since at least 2005.[^1]


[^1]: [`SRC-MCSHERRY-EFF-BIO`](../../../bibliography.md#SRC-MCSHERRY-EFF-BIO) — Corynne McSherry EFF Staff Biography. Self-reported profile; corroboration pending.

## Relevance to Open Source and Software Companies

McSherry's expertise in copyright, intermediary liability, and open access directly addresses the legal landscape that open source companies navigate. Her work defending fair use and the public domain protects the ecosystem in which open source thrives. Companies facing copyright claims, DMCA takedowns, or challenges to their platforms' legal protections under intermediary liability doctrines would benefit from her expertise.

## Relationships

- `PERSON-CORYNNE-MCSHERRY` cites [`SRC-MCSHERRY-EFF-BIO`](../../../bibliography.md#SRC-MCSHERRY-EFF-BIO).
- `PERSON-CORYNNE-MCSHERRY` affiliated_with `ORG-EFF`.

## Sources

1. [`SRC-MCSHERRY-EFF-BIO`](../../../bibliography.md#SRC-MCSHERRY-EFF-BIO): Corynne McSherry EFF Staff Biography.

## Research Debt

- Document specific cases relevant to open source software.
- Add details on intermediary liability work and Section 230.
- Document her work on DMCA reform.
- Corroborate self-reported biography or about-page claims with independent sources (institutional profile, court docket, case page, or reputable journalism).
- Add primary or secondary sources for major litigation and impact claims (dockets, case pages, official court records, or reputable journalism).
