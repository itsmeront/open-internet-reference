---
id: ORG-OSI
title: Open Source Initiative
type: organization
status: draft
summary: The Open Source Initiative is a non-profit corporation with global scope formed to educate about and advocate for the benefits of open source software.
tags:
  - organization
  - open-source-software
  - civil-society
  - outreach
sources:
  - SRC-OSI-ABOUT
  - SRC-OSI-HISTORY
relationships:
  - subject: ORG-OSI
    predicate: cites
    object: SRC-OSI-ABOUT
    sources:
      - SRC-OSI-ABOUT
  - subject: ORG-OSI
    predicate: cites
    object: SRC-OSI-HISTORY
    sources:
      - SRC-OSI-HISTORY
last_verified: "2026-06-25"
---

# Generated Source Mirror

This page is generated from the source Markdown file so it can be viewed inside the MkDocs site.

- Source path: `knowledge/organizations/ORG-OSI.md`
- Source ID: `ORG-OSI`
- [**Edit this page**](https://github.com/itsmeront/open-internet-reference/edit/main/knowledge/organizations/ORG-OSI.md) | [**Suggest a change**](https://github.com/itsmeront/open-internet-reference/issues/new?template=suggest-edit.yml&title=Edit+suggestion:+ORG-OSI)

!!! info "📝 Page Status: Draft — sourced but not yet reviewed"

    - **Status**: `draft`
    - **Sources**: 2
    - **Relationships**: 2
    - **Research debt items**: 5
    - **Last verified**: 2026-06-25

---

# Open Source Initiative

## Summary

The Open Source Initiative is a non-profit corporation with global scope formed to educate about and advocate for the benefits of open source software.

## Verified Facts

- The official about page describes OSI as a non-profit corporation with global scope formed to educate about and advocate for the benefits of open source and to build bridges among different constituencies in the open source community.[^1]
- The official history page states OSI was jointly founded by Eric Raymond and Bruce Perens in late February 1998, with Raymond as its first president and Perens as vice-president.[^2]
- The initial Board of Directors included Brian Behlendorf, Ian Murdock, Russ Nelson, and Chip Salzenberg.[^1]
- OSI is the steward of the Open Source Definition, which sets the foundation for the open source software ecosystem.[^1]


[^1]: [`SRC-OSI-ABOUT`](../../../bibliography.md#SRC-OSI-ABOUT) — Open Source Initiative About Page. Self-reported profile; corroboration pending.
[^2]: [`SRC-OSI-HISTORY`](../../../bibliography.md#SRC-OSI-HISTORY) — Open Source Initiative History Page. Official source; review reliability tier.

## Public Contact Information

- Website: https://opensource.org/
- About: https://opensource.org/about
- Organization: https://opensource.org/organization

## Relevance to Open Source and Software Companies

OSI is the authoritative body that maintains the Open Source Definition, which defines what qualifies as an open source license. Companies releasing open source software or using open source licensing benefit from OSI's educational resources, advocacy, and network of affiliate organizations. OSI's work helps establish and protect the legal and community norms around open source.

## Relationships

- `ORG-OSI` cites [`SRC-OSI-ABOUT`](../../../bibliography.md#SRC-OSI-ABOUT).
- `ORG-OSI` cites [`SRC-OSI-HISTORY`](../../../bibliography.md#SRC-OSI-HISTORY).

## Sources

1. [`SRC-OSI-ABOUT`](../../../bibliography.md#SRC-OSI-ABOUT): Open Source Initiative About Page.
2. [`SRC-OSI-HISTORY`](../../../bibliography.md#SRC-OSI-HISTORY): Open Source Initiative History Page.

## Research Debt

- Add details on the Open Source Definition and its legal significance.
- Document OSI's affiliate organizations network.
- Add information about license review process relevant to software companies.
- Corroborate self-reported biography or about-page claims with independent sources (institutional profile, court docket, case page, or reputable journalism).
- Review per-fact footnote-to-source mapping; multiple sources are cited on this page.
