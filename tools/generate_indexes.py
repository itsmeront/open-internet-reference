...
def record_for(path: Path, metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "charge_date": metadata.get("charge_date"),
        "arrest_date": metadata.get("arrest_date"),
        "indictment_date": metadata.get("indictment_date"),
        "path": relative_path,
...
def timeline_events(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Build public timeline entries from typed legal/historical date fields.

    Document publication and verification dates belong on page footers, not here.
    """
    events: list[dict[str, Any]] = []
    for record in records:
        summary = record.get("summary") or record.get("title") or record["id"]
        for field_name, event_type, _label in TIMELINE_DATE_FIELDS:
...
def write_timeline_markdown(
    ...
    events = timeline_events(records)
    kind_labels = {event_type: label for _field, event_type, label in TIMELINE_DATE_FIELDS}
    lines = [
        "# Generated Timeline",
        "",
        (
    ...
    if not events:
        lines.extend(
            [
                "_No records with timeline dates are available yet._",
                "",
            ]
        )
    ...
    for event in events:
        ...
        kind_label = kind_labels.get(event["event_type"], event["event_type"])
        lines.append(
            f"- **{markdown_escape(kind_label)}**: {record_link} — "
            f"{markdown_escape(event['description'])}"
        )
...
def _document_metadata_footer(record: dict[str, Any]) -> list[str]:
    """Append document dating metadata at the bottom of mirrored pages."""
    lines = [
        "",
        "## Document metadata",
        "",
    ...
    if _is_meaningful_metadata_value(str(event_date) if event_date is not None else None):
        lines.append(f"- Event date: `{event_date}`")

    for field_name, _kind, label in TIMELINE_DATE_FIELDS:
        if field_name == "event_date":
            continue
        value = record.get(field_name)
        if _is_meaningful_metadata_value(str(value) if value is not None else None):
            lines.append(f"- {label} date: `{value}`")

    last_verified = record.get("last_verified")
    if _is_meaningful_metadata_value(str(last_verified) if last_verified is not None else None):
        lines.append(f"- Last verified: `{last_verified}`")
    else:
        lines.append("- Last verified: _not recorded_")
    ...
    if isinstance(body_metadata, dict):
        for key, label in (
            ...
            value = body_metadata.get(key)
            if isinstance(value, str) and _is_meaningful_metadata_value(value):
...
def write_source_doc_mirror(
    ...
    footer = _document_metadata_footer(record)
    ...
    if original.startswith("---\n"):
        ...
        lines = [
            "---",
            *raw_metadata.rstrip().splitlines(),
            "---",
            "",
            *notice,
            *body_lines,
