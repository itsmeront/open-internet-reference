...
def timeline_events(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Build public timeline entries from historical-event `event_date` values.
...
def timeline_payload(records: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "schema_version": 2,
        "description": (
            "Generated timeline of historical events from explicit `event_date` "
            "front-matter values. Document publication and verification dates "
            "appear on individual page footers instead."
        ),
...
def write_timeline_markdown(
    ...
    lines = [
        ...
        "",
        (
            "This page lists historical events in chronological order using each "
            "record's `event_date`. Document publication and verification dates "
            "are shown at the bottom of individual pages. Do not edit this file "
            "manually."
        ),
        "",
    ...
    if not events:
        lines.extend(
            [
                "_No historical events with an `event_date` are available yet._",
                "",
            ]
        )
    ...
    for event in events:
        ...
        lines.append(
            f"- {record_link} — {markdown_escape(event['description'])}"
        )
